...
- 自有 Clash 转换后交由 KOP server 路径完成最终 Base64 返回。
- KOP 其他服务器格式回退。
- 自有解析失败后的 KOP 回退。
- KOP server 参数应用。
- KOP `filter` 和 `rewrite` 路径。
- KOP 参数助手注册。
- 自有转换 bundle 不调用 `$done`；KOP 路径保留上游直接回调行为。
...
通过 iPhone 镜像复测确认，三个不同资源的公共失败点是合并版间接调用 `$done`：自有路径通过 `doneOnce` 调用，KOP 路径先截获结果再调用。Node VM 中这种调用等价，但它偏离了官方示例。版本 `31ed531` 恢复 KOP 在条件顶层块内直接回调后，用户真机确认 `filter` 和 `rewrite` 已正常；但自有 server 路径继续直接返回相同的 61 行原生节点文本时，`westdata3` 仍报 `Result type error`。安全对比确认该文本与历史真机成功版本逐字节一致。版本 `bb9917c` 将自有转换结果交给 KOP 的顶层 server 流程统一完成最终 Base64 返回，用户已在真机确认 `westdata3` 可以正常导入和使用。

当前混合版本已由用户真机确认：自有 YAML 节点、KOP 分流和 KOP 重写均正常。后续运行时变更仍需区分本地测试与 Quantumult X 真机验证。
